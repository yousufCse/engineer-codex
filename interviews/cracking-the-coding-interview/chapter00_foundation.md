...
```mermaid
flowchart TD
    A["1️⃣ Listen<br/>মন দিয়ে শোনো"] --> B["2️⃣ Draw Example<br/>উদাহরণ আঁকো"]
    B --> C["3️⃣ Brute Force<br/>প্রথম সমাধান বলো"]
    C --> D["4️⃣ Optimize (BUD)<br/>উন্নত করো"]
    D --> E["5️⃣ Walk Through<br/>মুখে বুঝিয়ে বলো"]
    E --> F["6️⃣ Implement<br/>code লেখো"]
    F --> G["7️⃣ Test<br/>edge case দিয়ে যাচাই"]
    G -->|bug পেলে| F
```

চলুন প্রতিটা ধাপ বুঝি:

### 1️⃣ Listen — মন দিয়ে শোনো
...
### 2️⃣ Draw an Example — বড়, concrete উদাহরণ আঁকো
...
### 3️⃣ State the Brute Force — প্রথম যে সমাধান মাথায় আসে, সেটাই বলুন
Brute force খারাপ/ধীর হলেও সমস্যা নেই। এটা দেখায় আপনি problem টা বুঝেছেন, আর এটাই optimize করার starting point.
> ⚠️ "এটা খুব slow" বলে চুপ থাকবেন না — slow solution-ও বলে ফেলুন, পরে ঠিক করবেন।

### 4️⃣ Optimize — BUD দিয়ে উন্নত করুন
...
### 5️⃣ Walk Through — code লেখার আগে মুখে বুঝিয়ে বলুন
...
> ⚠️ Big-O **সেকেন্ড মাপে না**। এটা মাপে **growth rate** — input `n` বড় হলে কাজ কীভাবে বাড়ে। কারণ দ্রুত আর ধীর কম্পিউটারে সেকেন্ড আলাদা, কিন্তু growth pattern একই।
...
```
> ⚠️ **সাবধান:** loop-এ n বার চললে time O(n) কিন্তু space O(1)। Recursion-এ n বার ডাকলে time O(n) **এবং** space O(n) (call stack)।
